...
public:
	...
	MemberId(TypeId id, uint32_t Offset) : Id{ id }, Offset{ Offset } { };

	MemberId()									= default;
	~MemberId()								= default;
	MemberId(const MemberId&)					= default;
	MemberId& operator=(const MemberId&)		= default;
	MemberId(MemberId&&) noexcept				= default;
	MemberId& operator=(MemberId&&) noexcept	= default;
